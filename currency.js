...
document.getElementById(
    "dateInfo"
).innerHTML = `${day} ${months[month]} ${year}`;
...
function calculateCurrency() {
    ...
    fetch(url)
        ...
        .then((data) => {
            ...
            var exchangeRateFixed = (exchangeRate * 1).toFixed(2);
            ...
            document.getElementById(
                "alt"
            ).innerHTML = `Exchange Rate :  1 ${fromCurrencyCode}  =  ${exchangeRateFixed} ${toCurrencyCode}`;

            document.getElementById(
                "tutarAlt"
            ).innerHTML = `${tutarDoviz} ${fromCurrencyCode} = ${sonuc} ${toCurrencyCode}`;
            // document.getElementById(
            //     "cevrilenAlt"
            // ).innerHTML = `Çevrilen Tutar : ${sonuc} ${toCurrencyCode}`;
        })
